 ...
 */
export default function AdminOrderDetailPage({ params }: { params: PageParams }) {
   ...
   */
  const fetchArtworkDetails = async (items: any[]) => {
    try {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded">
          ...
          <Link href="/admin/orders" className="mt-2 text-sm underline block">
            Back to Orders
          </Link>
        </div>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-8">
        <div>
          ...
          <p className="text-gray-500 font-mono mt-1">ID: {order.id}</p>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            ...
            <ul className="divide-y divide-gray-200">
              {order.items?.map((item: any, index: number) => {
                ...
                const artworkImage = artwork?.images?.[0]?.url || '/placeholder-artwork.jpg';
